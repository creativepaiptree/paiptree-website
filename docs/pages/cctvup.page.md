...
- `관리 농장` 탭은 현재 농장 그룹 목록을 업체별 섹션으로 재구성한다. 업체 그룹은 신우, 체리부로, CPF, PRIFOODS, 해외, 기타 순서다. `POC`는 정식 category가 아니라 Supabase registry의 `tags` 값으로 관리하고, 태그가 있는 농장은 `POC` 표식을 보여준다.
- `관리 농장` 탭에서 `관제` 버튼을 누르면 해당 농장을 펼친 상태로 `실시간 관제` 탭에 진입한다.
...
- Phase 3.20: `관리 농장` 탭과 `/api/cctvup/managed-farms` bridge API를 추가한다. 현재 반영 완료. 화면은 기존 current list와 Supabase registry 표시값을 합쳐 업체/태그/감시범위 기준 목록을 보여주고, 최종 Supabase wrapper/view 확정 시 route 내부 원천만 교체한다.
...
## 6. 인터랙션 규칙
- 주요 사용자 액션: 운영 점검, 지금 체크, Supabase 진단, 테마 전환, 전체/정상/문제 농장 필터링, 카테고리 다중 필터링, 감시범위 다중 필터링, 농장 정렬 변경, 검색, 농장 펼침/접힘, 카메라 행 선택, 관리 농장 업체/감시범위/상태 필터링, 관리 농장 행의 관제 이동, 자동 5분 재조회
...
- [x] `관리 농장` 탭이 업체/태그/감시범위 기준 농장 목록을 표시하는지 확인
