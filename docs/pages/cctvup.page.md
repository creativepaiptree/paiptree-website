...
- `관리 농장` 탭은 현재 농장 그룹 목록을 업체별 가로 레인으로 재구성한다. 업체 그룹은 체리부로, 신우, CPF, PRIFOODS, 해외, 기타 순서다. 각 레인 안에는 농장 카드가 세로로 쌓이며, `POC`는 정식 category가 아니라 Supabase registry의 `tags` 값으로 관리하고 태그가 있는 농장은 `POC` 표식을 보여준다.
- `관리 농장` 탭에서 농장 카드의 `관제` 버튼을 누르면 해당 농장을 펼친 상태로 `실시간 관제` 탭에 진입한다.
...
- 소속 기준: 운영 DB의 `tbl_farm_service.affiliates`와 `country`를 우선 사용해 신우/체리부로/해외/기타 기본 분류를 잡고, 사용자가 화면에서 바꾼 Supabase registry 값을 최종 override로 사용한다. 관리 농장 화면의 업체 그룹은 category와 별개로 계산하며, 태국 계열은 `CPF`, 일본 계열은 `PRIFOODS`로 표시한다. `POC`는 category가 아니라 registry `tags`로 관리한다.
...
## 6. 인터랙션 규칙
- 주요 사용자 액션: 운영 점검, 지금 체크, Supabase 진단, 테마 전환, 전체/정상/문제 농장 필터링, 카테고리 다중 필터링, 감시범위 다중 필터링, 농장 정렬 변경, 검색, 농장 펼침/접힘, 카메라 행 선택, 관리 농장 업체/감시범위/상태 필터링, 관리 농장 카드의 관제 이동, 자동 5분 재조회
