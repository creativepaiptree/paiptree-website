...
- `관리 농장` 탭은 현재 농장 그룹 목록을 업체별 가로 레인으로 재구성한다. 업체 그룹은 체리부로, 신우, CPF, PRIFOODS, 대만, 기타 순서다. 업체 체크박스는 `체리부로`, `신우`가 기본 선택되어 시작하고, 추가 업체를 체크하면 해당 업체 레인을 함께 보여준다. 각 레인 안에는 compact 농장 카드가 세로로 쌓이며, `POC`는 정식 category가 아니라 Supabase registry의 `tags` 값으로 관리한다.
...
## 6. 인터랙션 규칙
- 주요 사용자 액션: 운영 점검, 지금 체크, Supabase 진단, 테마 전환, 전체/정상/문제 농장 필터링, 카테고리 다중 필터링, 감시범위 다중 필터링, 농장 정렬 변경, 검색, 농장 펼침/접힘, 카메라 행 선택, 관리 농장 업체 체크/업체/감시범위/상태 필터링, 관리 농장 카드 클릭 이동, 자동 5분 재조회
