...
- `관리 농장` 탭은 현재 농장 그룹 목록을 업체별 가로 레인으로 재구성한다. 업체 그룹은 체리부로, 신우, CPF, PRIFOODS, 대만, 기타 순서다. 기본 범위 select는 `체리부로·신우`로 시작하고, `전체 보기`를 선택하면 나머지 업체 레인까지 함께 보여준다. 각 레인 안에는 compact 농장 카드가 세로로 쌓이며, `POC`는 정식 category가 아니라 Supabase registry의 `tags` 값으로 관리한다.
...
- 공통 내부 도구 메뉴(`DashFloatingNav`)를 좌측에 제공한다. 기본 상태는 접힘이며, 펼쳤을 때 `/cctvup`이 현재 페이지로 표시된다.
...
- Phase 3.20: `관리 농장` 탭과 `/api/cctvup/managed-farms` bridge API를 추가한다. 현재 반영 완료. 화면은 기존 current list와 Supabase registry 표시값을 합쳐 업체별 가로 레인과 농장 카드로 보여주고, 최종 Supabase wrapper/view 확정 시 route 내부 원천만 교체한다.
...
## 6. 인터랙션 규칙
- 주요 사용자 액션: 운영 점검, 지금 체크, Supabase 진단, 테마 전환, 전체/정상/문제 농장 필터링, 카테고리 다중 필터링, 감시범위 다중 필터링, 농장 정렬 변경, 검색, 농장 펼침/접힘, 카메라 행 선택, 관리 농장 기본 범위/업체/감시범위/상태 필터링, 관리 농장 카드 클릭 이동, 자동 5분 재조회
