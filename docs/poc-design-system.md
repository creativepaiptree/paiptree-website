...
새 컴포넌트에서 라이트/다크 테마를 모두 지원하는 방법.

**원칙:** `<style jsx>` 내부는 반드시 CSS 변수 사용. Tailwind 인라인은 `globals.css` `.poc-theme-light` 오버라이드가 처리.

```tsx
// 1. themeMode prop 받기
type Props = { themeMode: 'dark' | 'light' }

// 2. 상태 색상만 THEME 객체로 분기
const THEME = {
...
// 4. 상태 색상만 인라인 style 또는 THEME 객체 사용
<span style={{ color: theme.textOK }}>정상</span>
<span style={{ color: theme.textDanger }}>오류</span>
```
