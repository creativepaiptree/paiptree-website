...
### 3-4. 콘텐츠 영역 수평 여백 (Content Inset)

**원칙:** 헤더·툴바 등 페이지 크롬은 edge-to-edge 유지. 그 아래 콘텐츠 영역은 **반드시 여백**을 적용해 콘텐츠가 브라우저 좌우 끝에 직접 닿지 않도록 한다.

| 패턴 | 적용 대상 | 래퍼 구조 |
|---|---|---|
| **A — 단일 스크롤** | PoC 대시보드 | `flex-1 overflow-y-auto p-4` → `max-w-[1760px] mx-auto` |
| **B — 전체 너비 도구** | i18n, 전체 폭 테이블 | `flex-1 overflow-hidden p-3` → `h-full overflow-auto border border-[#30363d]` |
| **C — 패널 분할** | tms/main, farm (사이드바+메인) | `flex flex-1 overflow-hidden p-3 gap-3` → 각 패널 `border border-[#30363d]` |
...
// ── 패턴 B (전체 너비 도구 테이블) ─────────────────────────────────────
// p-3: 외곽 여백, border: 박스 테두리 — 4면 모두 표시
...
// ── 패턴 C (패널 분할 레이아웃) ─────────────────────────────────────────
// 각 패널은 border-r / border-l 단독 사용 금지 — border 전체 테두리 적용
<div className="flex flex-1 overflow-hidden p-3 gap-3">
  <aside className="w-56 shrink-0 border border-[#30363d] bg-[#161b22] flex flex-col">
    {/* 좌측 사이드바 */}
  </aside>
  <main className="flex-1 flex flex-col overflow-hidden border border-[#30363d]">
    {/* 메인 콘텐츠 */}
  </main>
  <aside className="w-64 shrink-0 border border-[#30363d] bg-[#161b22] flex flex-col">
    {/* 우측 인스펙터 (있을 경우) */}
  </aside>
</div>
```

> **패턴 C 주의:** `p-3 gap-3` 조합이면 패널 간격 12px + 외곽 여백 12px. 더 넓게 할 경우 `p-4 gap-4` 사용.
