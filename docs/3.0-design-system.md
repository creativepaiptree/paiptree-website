...
// 6. 고정 사이드 드로어 (클릭 시 우측에서 열리는 상세 패널 — 모든 시스템 공통)
{isOpen && (
  <>
    ...
    <aside className="fixed right-0 top-0 z-50 h-full w-80 border-l border-[#30363d] bg-[#161b22] shadow-2xl flex flex-col">
      <div className="flex items-start justify-between px-4 py-3 border-b border-[#30363d] shrink-0">
        <h2 className="text-sm font-semibold text-[#c9d1d9]">패널 제목</h2>
        <button type="button" onClick={onClose}
          className="inline-flex items-center border border-[#30363d] px-2 py-1 text-xs text-[#8b949e] hover:bg-[#21262d] transition-colors">✕</button>
      </div>
      <div className="flex-1 overflow-y-auto p-4">{/* 본문 */}</div>
    </aside>
  </>
)}
```
...
### 4-7. Side Drawer (고정 사이드 드로어) ← 모든 시스템 기본 패턴

노드/행/아이템 클릭 시 우측에서 열리는 상세 패널. **PoC TracePanel, dash Service Architecture Inspector 모두 이 구조를 따른다.**
새 페이지에서 상세 정보 패널이 필요할 때 반드시 이 패턴을 사용한다.

```tsx
// ── z-index 레이어 체계 ─────────────────────────────────────────────────
// z-40  : backdrop (클릭 시 닫힘)
// z-50  : 사이드 드로어 / 중앙 모달 패널
// z-[9999] : DashFloatingNav (항상 최상위)
    ...
    <aside className="fixed right-0 top-0 z-50 h-full w-80 border-l border-[#30363d] bg-[#161b22] shadow-2xl flex flex-col">
      ...
      <div className="flex items-start justify-between px-4 py-3 border-b border-[#30363d] shrink-0">
        <div className="flex flex-col gap-1.5 min-w-0">
          {/* 배지 (선택) */}
          <span className="text-[9px] font-mono border px-1 py-[1px] uppercase tracking-widest self-start
                           text-[#58a6ff] border-[#58a6ff]">
            LABEL
          </span>
          <h2 className="text-sm font-bold font-mono text-[#c9d1d9]">제목</h2>
          <p className="text-[10px] text-[#8b949e] font-mono">부제목</p>
        </div>
        <button
          type="button"
          onClick={onClose}
          className="inline-flex items-center border border-[#30363d] px-2 py-1 text-xs
                     text-[#8b949e] hover:bg-[#21262d] transition-colors shrink-0 ml-3 mt-0.5"
        >
          ✕
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        <p className="text-[12px] text-[#c9d1d9] leading-relaxed">설명 텍스트</p>

        {/* 연결 목록 등 부가 섹션 */}
        <div>
          <p className="text-[9px] font-mono text-[#6e7681] uppercase tracking-widest mb-2">섹션 제목</p>
          <div className="flex flex-col gap-1.5">
            <span className="text-[11px] font-mono text-[#8b949e] border-l-2 border-[#30363d] pl-3">항목</span>
          </div>
        </div>

        {/* 하단 액션 링크 (선택) */}
        <a href="/path"
           className="text-[11px] font-mono border border-[#30363d] px-3 py-2
                      text-[#58a6ff] hover:bg-[#21262d] transition-colors text-center block">
          → 페이지로 이동
        </a>
      </div>
...
| 용도 | 너비 |
|---|---|
| 일반 상세 패널 (노드/행 클릭) | `w-80` (320px) |
| 데이터 추적 (TracePanel) | `max-w-[760px]` |
| 좁은 인스펙터 | `w-64` (256px) |

> **원칙:** 사이드 드로어는 항상 `fixed` 포지셔닝을 사용한다. 메인 콘텐츠(SVG, 테이블 등)의 크기에 영향을 주지 않는 것이 핵심이다. `absolute`로 구현하면 부모 크기 변화나 스크롤 위치에 영향을 받으므로 금지한다.
