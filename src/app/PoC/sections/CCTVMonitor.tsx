...
const CCTVMonitor = ({ lang, onOpenTrace }: CCTVMonitorProps) => {
  ...
  const selectedImageTraces = useMemo(() => {
    if (!selectedImage) {
      return null;
    }

    const baseSource = {
      source_id: `file:archive:${selectedImage.id}`,
      type: 'file' as const,
      name: `processed_frame_${selectedImage.id}.jpg`,
      url: selectedImage.processedImageUrl,
      highlight_text: `${selectedImage.capturedAt} / ${selectedImage.processedAt}`,
      highlight_anchor: `frame=${selectedImage.sequence + 1}`,
    };

    return {
      frame: {
        trace_id: `cctv:selected-frame:${selectedImage.id}`,
        display_value: `#${selectedImage.sequence + 1}/${BATCH_FRAME_COUNT}`,
        logic_summary:
          lang === 'ko'
            ? '현재 선택된 보관 가공 프레임 위치입니다.'
            : 'Current selected archived processed frame index.',
        logic_formula: lang === 'ko' ? '선택 프레임 번호 / 배치 총 프레임 수' : 'selected frame / total batch frames',
        data_source: [baseSource],
        is_ai_generated: false,
        source_version: 'archive-v1',
        snapshot_at: selectedImage.processedAt,
      } satisfies TraceabilityPayload,
      capturedAt: {
        trace_id: `cctv:captured-at:${selectedImage.id}`,
        display_value: selectedImage.capturedAt,
        logic_summary:
          lang === 'ko'
            ? '원본 프레임 캡처 시각입니다.'
            : 'Raw frame capture timestamp.',
        logic_formula: lang === 'ko' ? '캡처 시각 = 카메라 타임스탬프' : 'captured_at = camera timestamp',
        data_source: [baseSource],
        is_ai_generated: false,
        source_version: 'archive-v1',
        snapshot_at: selectedImage.capturedAt,
      } satisfies TraceabilityPayload,
      processedAt: {
        trace_id: `cctv:processed-at:${selectedImage.id}`,
        display_value: selectedImage.processedAt,
        logic_summary:
          lang === 'ko'
            ? '가공 파이프라인 처리 완료 시각입니다.'
            : 'Processing pipeline completion timestamp.',
        logic_formula: lang === 'ko' ? '가공 시각 = 분석 엔진 완료 시각' : 'processed_at = analytics completion timestamp',
        data_source: [baseSource],
        is_ai_generated: true,
        source_version: 'vision-pipeline-1.8.0',
        snapshot_at: selectedImage.processedAt,
        confidence: selectedImage.score,
        version_history: [
          {
            source_version: 'vision-pipeline-1.7.2',
            snapshot_at: selectedImage.capturedAt,
            display_value: selectedImage.processedAt,
            logic_summary:
              lang === 'ko'
                ? '이전 파이프라인 버전에서 생성된 처리 시각입니다.'
                : 'Processing timestamp generated by previous pipeline version.',
            confidence:
              selectedImage.score !== undefined ? Math.max(selectedImage.score - 0.05, 0) : undefined,
            data_source: [
              {
                source_id: `file:archive:${selectedImage.id}:legacy`,
                type: 'file',
                name: `processed_frame_${selectedImage.id}_legacy.jpg`,
                url: selectedImage.processedImageUrl,
                highlight_text: `${selectedImage.capturedAt} / legacy`,
                highlight_anchor: `frame=${selectedImage.sequence + 1}`,
              },
            ],
          },
      ...
      imageUrl: {
        trace_id: `cctv:image-url:${selectedImage.id}`,
        display_value: selectedImage.processedImageUrl,
        logic_summary:
          lang === 'ko'
            ? '선택 프레임의 가공 이미지 경로입니다.'
            : 'Processed image URL for the selected frame.',
        logic_formula: lang === 'ko' ? 'URL = 저장 경로 + 프레임 파일명' : 'URL = storage path + frame filename',
        data_source: [baseSource],
        is_ai_generated: false,
        source_version: 'archive-v1',
        snapshot_at: selectedImage.processedAt,
  ...
  const liveStreamTrace = useMemo<TraceabilityPayload | null>(() => {
    if (!liveState) {
      return null;
    }

    return {
      trace_id: `cctv:live-stream:${activeCamera}`,
      display_value: liveState.streamUrl,
      logic_summary:
        lang === 'ko'
          ? `${activeCamera} 카메라 실시간 스트림 주소입니다.`
          : `Realtime stream endpoint for camera ${activeCamera}.`,
      logic_formula: lang === 'ko' ? 'stream_url = camera_gateway + channel_id' : 'stream_url = camera_gateway + channel_id',
      data_source: [
        {
          source_id: `api:live-stream:${activeCamera}`,
          type: 'api',
          name: 'live_stream_registry',
          url: 'https://p-root.local/api/live-stream-registry',
          highlight_text: liveState.streamUrl,
          highlight_anchor: `camera=${activeCamera}`,
        },
      ],
      is_ai_generated: false,
      source_version: 'stream-registry-v1',
      snapshot_at: liveState.lastHeartbeat,
      version_history: [
        {
          source_version: 'stream-registry-v0',
          snapshot_at: '13:45',
          display_value: liveState.streamUrl,
          logic_summary:
            lang === 'ko'
              ? '이전 레지스트리 동기화 시점의 스트림 엔드포인트입니다.'
              : 'Stream endpoint from previous registry sync point.',
          data_source: [
            {
              source_id: `api:live-stream:${activeCamera}:legacy`,
              type: 'api',
              name: 'live_stream_registry_legacy',
              url: 'https://p-root.local/api/live-stream-registry?version=v0',
              highlight_text: liveState.streamUrl,
              highlight_anchor: `camera=${activeCamera}`,
            },
          ],
                     ...
                     >
                       <div className="thumb-preview">
                         ...
                         <div className="thumb-overlay">
                           <span className="thumb-chip thumb-time">{img.capturedAt}</span>
                           <span className="thumb-chip" style={{ color: compactColor, borderColor: compactColor }}>
                             {compactLabel}
                           </span>
                         </div>
                       </div>
