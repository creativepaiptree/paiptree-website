...
import type { CherryTmsSettlementRegisterPageData, CherryTmsSettlementRegisterRow } from '@/lib/cherryTmsSettlementRegister';

const buildRegisterHref = (month?: string | null, date?: string | null) => {
  const params = new URLSearchParams();
  if (month) params.set('month', month);
  if (date) params.set('date', date);
  const query = params.toString();
  return query ? `/cherry_tms/settlement-register/?${query}` : '/cherry_tms/settlement-register/';
};
...
const filterRows = (rows: CherryTmsSettlementRegisterRow[], month: string | null, date: string | null) => {
  if (date) return rows.filter((row) => row.dispatchDate === date);
  if (month) return rows.filter((row) => row.dispatchDate.startsWith(month));
  return rows;
};

export function CherryTmsSettlementRegisterClient({
  data,
  fallbackRows,
}: {
  data: CherryTmsSettlementRegisterPageData | null;
  fallbackRows: CherryTmsSettlementRegisterRow[];
}) {
  ...
  const filteredRows = useMemo(() => filterRows(rowsForBase, monthParam, dateParam), [rowsForBase, monthParam, dateParam]);
  ...
  const selectedRow = filteredRows[0];
  const statsForRender = hasSupabaseData
    ? [
        { label: '원천 행', value: `${data.sourceRowCount.toLocaleString('ko-KR')}건`, hint: 'Supabase raw Excel rows' },
        { label: '등록 후보', value: `${filteredRows.length.toLocaleString('ko-KR')}묶음`, hint: 'grouping에서 넘어온 정산 등록 후보' },
        { label: '검토 필요', value: `${data.manualReviewCount.toLocaleString('ko-KR')}건`, hint: '수동 확인이 필요한 원천/detail' },
        { label: '연결 상태', value: '실데이터', hint: 'tbl_tms_cherrybro_grouping_page_v1 기반' },
      ]
    : registerStats;
  const executionItems = [
    ['기준일자', data?.groupingDate ?? selectedRow?.dispatchDate ?? '2026-04-23'],
    ['선택 기사', selectedRow?.driver || '-'],
    ['차량/소속', selectedRow?.vehicle || '-'],
    ['등록 기준', 'grouping row → settlement candidate'],
    ['현재 상태', hasSupabaseData ? `${filteredRows.length.toLocaleString('ko-KR')}건 등록 후보` : '샘플 fallback'],
    ['다음 단계', '정산 검토로 전달'],
  ];
  ...
  return (
    ...
    >
      ...
      {hasSupabaseData ? (
        <section className="border border-[#243041] bg-[#0b1220]">
          <div className="border-b border-[#243041] bg-[#0f1722] px-4 py-3">
            <h2 className="text-lg font-semibold text-white">월 / 일자 필터</h2>
          </div>
          <div className="grid gap-4 px-4 py-4 lg:grid-cols-2">
            <div className="grid gap-3">
              <p className="text-xs uppercase tracking-[0.16em] text-slate-500">월</p>
              <div className="flex flex-wrap gap-2">
                <Link
                  href={buildRegisterHref(null, null)}
                  className={`border px-3 py-2 text-sm transition ${!monthParam && !dateParam ? 'border-[#4D7CFF] bg-[#1c2c52] text-white' : 'border-[#314056] bg-[#0a1019] text-slate-200 hover:bg-white/5'}`}
                >
                  전체
                </Link>
                {visibleMonths.map((month) => (
                  <Link
                    key={month}
                    href={monthParam === month && !dateParam ? buildRegisterHref(null, null) : buildRegisterHref(month, null)}
            ...
            <div className="grid gap-3">
              <p className="text-xs uppercase tracking-[0.16em] text-slate-500">일자</p>
              <div className="flex flex-wrap gap-2">
                <Link
                  href={buildRegisterHref(selectedMonth, null)}
                  className={`border px-3 py-2 text-sm transition ${!dateParam ? 'border-[#4D7CFF] bg-[#1c2c52] text-white' : 'border-[#314056] bg-[#0a1019] text-slate-200 hover:bg-white/5'}`}
                >
                  전체
                </Link>
                {visibleDates.map((date) => (
                  <Link
                    key={date}
                    href={dateParam === date ? buildRegisterHref(selectedMonth, null) : buildRegisterHref(selectedMonth, date)}
